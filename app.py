...
import os
class I_N_AndroidShell:
    def __init__(self, root):
        self.root = root
        self.root.title("Ink Nyto Android Shell")
        ...
        self.scroll_content = tk.Frame(canvas)
        canvas.create_window((0, 0), window=self.scroll_content, anchor="nw")
        
        # Create buttons
        self.outside_button = tk.Button(self.scroll_content, text="Input Text", command=self.open_text_dialog)
        self.outside_button.grid(row=0, column=0, padx=10, pady=10)
        
        self.shell_button = tk.Button(self.scroll_content, text="$#£||", command=self.open_shell_dialog)
        self.shell_button.grid(row=0, column=1, padx=10, pady=10)
        ...
        for key, value in buttons.items():
            button = tk.Button(self.scroll_content, text=f"{key} ({value.split(' ')[-1]})", command=lambda k=key: self.key_event_clicked(k, buttons))
    ...
    def shell(self, text):
        # Implement shell function here
        pass

    def open_text_dialog(self):
        text = tk.simpledialog.askstring("Input Text", "Enter the text:")
        if text is not None:
            self.adb_input_text(text)
    
    def open_shell_dialog(self):
        text = tk.simpledialog.askstring("Shell Command", "Enter the shell command:")
        if text is not None:
            self.shell(text)
    
    def adb_input_text(self, text):
        # Execute adb shell input text command
        os.system(f"adb shell input text '{text}'")
        print("Text entered:", text)

    def key_event_clicked(self, key, buttons):
        command = buttons.get(key)
        if command:
